...
echo -e "SPARK_MASTER_IP=master" >> spark-env.sh
...
echo "3.2 start config slaves file"
echo "slave1" > slaves
echo "slave2" >> slaves
if [ $? -ne 0 ]; then
  exit 1
...
#cd ${CUR_DIR}
tar -czvf ${CUR_DIR}/../../roles/spark/files/spark-${SPARK_VERSION}-bin-hadoop${version%.*}.tar.gz  spark-${SPARK_VERSION}-bin-hadoop${version%.*}/
if [ $? -ne 0 ]; then
  exit 1
fi
...
echo "6 create spark varfile"
echo "---" > ${CUR_DIR}/../../roles/spark/vars/spark_vars.yml
echo "SPARK_VERSION: $SPARK_VERSION" >> ${CUR_DIR}/../../roles/spark/vars/main.yml
echo "HADOOP_BIG_VERSION: ${version%.*}" >> ${CUR_DIR}/../../roles/spark/vars/main.yml
#echo ""
if [ $? -ne 0 ]; then
  exit 1
fi
